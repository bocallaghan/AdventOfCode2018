(function () {
    ...
    var common = require('./common.js'),
        ...
        solveFrequencyVariation = function (puzzleInput) {
            ...
            var overallResult = 0,
                ...
                // Then we split it by each , into an array
                frequencyChanges = cleanedPuzzleInput.split(","),
                i;

            // For every element in the array we extract the first character
            // which should be a plus or a minus. We then read the value after that
            // character and then add or subtract that value to the overall result.
            for (i = 0; i < frequencyChanges.length; i = i + 1){
                var incrementalValue = parseInt(frequencyChanges[i].substr(1));

                if (frequencyChanges[i].charAt(0) === "+"){
    ...
    common.logBanner("1a");
    common.runTestSuite(solveFrequencyVariation, testInputs, testOutputs);
    ...
    examResult = solveFrequencyVariation(examInput).toString();
